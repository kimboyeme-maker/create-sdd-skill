...
- Two Bundles whose writes reach the same paths — or whose writes reach the other's declared reads — can never run concurrently: the loop refuses that pair unless one depends on the other (`PROGRAM_WRITE_CONFLICT`), and `program-check` then reports the edge as `PROGRAM_ORDER_FORCED_BY_OVERLAP`. Cutting one package into two execution SDDs therefore buys an ordering constraint and a second control plane, never parallelism. Split where the write sets are disjoint, or keep one SDD and use batches.
...
- execution: {max_parallel, total_test_seconds, base_ref, allocations}. `max_parallel` is derived, not chosen: it is the width of the Bundle dependency graph — the largest number of Bundles that no ordering separates — and a smaller number is a scheduling decision that needs a stated reason. `program-check` reports `dependency_width` beside the declared value and says `PROGRAM_PARALLEL_UNDERDECLARED` when the plan asks for less than the graph allows. Allocations map every Bundle ID to its lifetime test seconds including all roles, rounds and retries, and sum to at most the total. These are proposed limits, never inferred user consent.
...
5. A copyable launch instruction in the user's language, for example `使用 sdd-loop-delivery 启动 <actual absolute total SDD path> 的完整 workflow`, with the real path substituted. It is an instruction for a host session, not a shell command; a host that renders follow-up actions may present it as one. Echo it in the reply every time, verbatim and on its own line, including when the documents were regenerated and nothing else changed: it is what the user acts on, and a reply that omits it hands back a finished program with no stated way to start. `repo-facts.ts check` on a program root returns it as `facts.launch`; print that string rather than composing one.
