...
- Two Bundles whose writes reach the same paths — or whose writes reach the other's declared reads — can never run concurrently unless one depends on the other, so the overlap forces an order. Cutting one package into two execution SDDs therefore buys an ordering constraint and a second control plane, never parallelism. Split where the write sets are disjoint, or keep one SDD and use batches.
...
Put one JSON block between `<!-- sdd-program:start -->` and `<!-- sdd-program:end -->` in the total SDD, separate from leaf sdd-contract blocks. The rules below define the fields; an assembled two-leaf index is in the [worked example](../examples/loop-ready-example.md#worked-program-index-multi-sdd-root).
...
Creating documents does not start implementation, tests, commits, merges or extra budget.
