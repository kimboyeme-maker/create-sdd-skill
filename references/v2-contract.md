...
`inventories` (optional) lists, per guarded or migrated requirement, the entry points that can break it: `[{"id":"I1","requirement":"R1","kind":"invariant|surface","statement":"…","entry_points":[{"name":"unUse","path":"packages/a/src/index.ts","acceptance":["A1"]} | {"name":"…","exempt":"why none is needed"}]}]`. An `invariant` inventory covers every mutating path of the protected state, including inverse transitions (enable, resume), managed or internal protocols, and replacement or restart; a `surface` inventory lists every declaration site of a migrated symbol (overloads, facades, type projections) with its `path`.
