...
A single SDD's report ends with one copyable launch instruction in the user's language, naming the document's absolute path, for example `使用 sdd-loop-delivery 启动 <absolute-SDD-path> 的交付`. Like the multi-SDD instruction in [program split](planning/program-split.md), it is an instruction for a host session, not a shell command, and a host that renders follow-up actions may present it as one. Give it whatever the host is, and never substitute a controller command: the loop's own entry takes the document path and an optional round bound, and role selection belongs to that skill. Reporting the instruction is not starting the run, and the user decides when to give it.
