...
## Acceptance
- A1 Given an index holding `a` and `b`, when `begin()`, `add("c")` and `rollback()` run and then `add("d")`, then `order()` equals that of a control index that never opened a transaction and received `add("d")`.
