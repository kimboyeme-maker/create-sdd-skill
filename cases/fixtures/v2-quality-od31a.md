...
Repaired sdd/v2 fixture for OD-31: a backticked arrow chain discriminates the quantifier. Each `v2-quality-od*.md`
...
## Acceptance
- A1 Given `A` ← `B` ← `C`, when `unUse('A')`, then it rejects with top-level `DEPENDENCY_BLOCKED`, `blockedBy` names every dependent in the chain, and the dry-run edge for absent D carries `'optional-absent'`.
