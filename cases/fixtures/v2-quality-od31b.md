...
Repaired sdd/v2 fixture for OD-31: a 、-separated enumeration discriminates the quantifier. Each `v2-quality-od*.md`
...
## Acceptance
- A1 Given dependents `B`、`C`、`D` of `A`, when `unUse('A')`, then it rejects with top-level `DEPENDENCY_BLOCKED`, `blockedBy` names them in order, and the dry-run edge for absent D carries `'optional-absent'`.
