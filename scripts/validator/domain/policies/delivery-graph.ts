...
function batchOrder(contract: Contract) {
  ...
  return {
    ...
    /**
     * Repeated lifecycle stages pair each consumer with a landed producer and require every
     * producer stage to reach a consumer. This keeps an early consumer before the first producer
     * invalid and prevents a late producer from escaping verification, without requiring the last
     * producer to precede the first lifecycle consumer.
     */
    staged: (producers: readonly string[], consumers: readonly string[]) =>
      consumers.every((consumer) =>
        producers.some((producer) => visit(consumer, new Set()).has(producer))
      ) &&
      producers.every((producer) =>
        consumers.some((consumer) => visit(consumer, new Set()).has(producer))
      )
  }
}
...
export function assertImplementationGraph(contract: Contract): void {
  ...
  for (const [consumerId, producers] of edges) {
    ...
    for (const producerId of producers) {
      ...
      if (!order.staged(producerBatches, consumerBatches))
        throw new Error(
  ...
  for (const path of paths) {
    ...
    const producerBatchGroups = [...edges.get(String(path.id))!].map((producerId) =>
      order.batchesOf(
        'requirement_ids',
    ...
    for (const acceptanceId of texts(path.acceptance_ids)
      ...
      if (
        ![requirementBatches, ...producerBatchGroups].every((producers) =>
          order.staged(producers, verifying)
        )
      )
        throw new Error(
