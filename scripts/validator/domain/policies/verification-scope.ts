...
export function assertVerificationScope(
  ...
  const targetedAssumptions = Array.isArray(targetIds)
    ? (payload.assumptions_checked as unknown[])
        .map(record)
  ...
  const routeInspection =
    targetedAssumptions.length > 0 &&
    targetedAssumptions.every((item) => item?.category === 'ROUTE_FEASIBILITY')
  const acceptanceMethod = scope.surfaces.some((value) => {
    const surface = record(value)!
    return (
  ...
  if (!migration && !routeInspection && !acceptanceMethod)
  ...
  for (const id of early.evidence_fact_ids) {
    ...
    if (
      ...
      fact.claim_ids.some((claim) => !text(claim)) ||
      new Set(fact.claim_ids).size !== fact.claim_ids.length
    )
      throw new Error('EARLY_FALSIFIER_CLAIM_BINDING_INVALID')
    if (!acceptanceMethod && routeInspection) {
      const source = record(fact.source)
      if (
        fact.claim_ids.length ||
        source?.kind !== 'SOURCE_INSPECTION' ||
        source.reference !== early.method
      )
        throw new Error('EARLY_FALSIFIER_ROUTE_INSPECTION_INVALID')
    }
  ...
  if ([...expectedPackages].some((owner) => !observedPackages.has(owner)))
    throw new Error('EARLY_FALSIFIER_FACT_SCOPE_INVALID')
  if (
    (!routeInspection || acceptanceMethod) &&
    [...expectedClaims].some((claim) => !observedClaims.has(claim))
  )
    throw new Error('EARLY_FALSIFIER_CLAIM_BINDING_INVALID')
