...
import { ORACLE_KINDS } from './differential-oracle'
 ...
 */
/** Sentence punctuation no shell command carries; the clearest signal that a method is prose. */
const METHOD_PROSE =
  /[\u3002\uff0c\u3001\uff1b\uff1a\uff1f\uff01\u2026\u201c\u201d\u2018\u2019\uff08\uff09]/
/** An executable or a path to one: what argv's first element has to be. */
const METHOD_HEAD = /^[A-Za-z0-9_.@:+\-/\\]+$/
/**
 * Whether a declared method can be run as a command at all. Deliberately shallow: the controller
 * cannot know which executables exist, and an inline script is a long, legitimate command, so this
 * rejects only what no command is — an opening word that is not an executable token, or sentence
 * punctuation — and lets anything command-shaped through to fail honestly at execution. An English
 * sentence whose first word happens to look like a program still passes; that part is create-sdd's
 * rule to keep, not something a shape check can decide.
 */
function executableMethod(method: unknown): boolean {
  if (typeof method !== 'string') return false
  const trimmed = method.trim()
  if (!trimmed || METHOD_PROSE.test(trimmed)) return false
  return METHOD_HEAD.test(trimmed.split(/\s+/)[0]!)
}
...
export function assertAcceptanceExecution(contract: Contract): void {
  ...
  for (const value of contract.acceptance) {
    ...
    // An unknown oracle kind would silently read as the mechanical default and bind the verdict to
    // an exit code the acceptance never meant.
    if (
      acceptance.oracle_kind !== undefined &&
      !ORACLE_KINDS.includes(acceptance.oracle_kind as (typeof ORACLE_KINDS)[number])
    )
      throw new Error(`ACCEPTANCE_ORACLE_KIND_INVALID: ${String(acceptance.id)}`)
    // A mechanical or differential oracle's method is the command the controller runs: `test-run`
    // binds argv to it and `execution_authorization` can only name methods the contract declares.
    // A narrative sentence there reads as a command everywhere — it is offered to the user as the
    // permission they are granting, and refused only once a lease is live and a round is underway.
    // Prose is what a judgment oracle is for, so the kind and the method have to agree here.
    if (acceptance.oracle_kind !== 'judgment' && !executableMethod(acceptance.method))
      throw new Error(
        `ACCEPTANCE_METHOD_NOT_EXECUTABLE: ${String(acceptance.id)} declares a ${String(
          acceptance.oracle_kind ?? 'mechanical'
        )} oracle, so its method is the command that runs; write the exact command, or declare oracle_kind "judgment" and state the pass condition the reviewer applies`
      )
    if (
