...
/**
 * Every migration reader has to fit inside one planned batch, checked while the plan can change.
 *
 * Admission binds each reader to a single execution packet and requires that packet to cover the
 * reader's requirements and acceptance (`MIGRATION_READER_PACKET_COVERAGE_INVALID`). Packets come
 * from the delivery plan's batches, so a reader whose requirements are split across two batches can
 * never be admitted — no packet exists that covers it. Discovering that at admission means a run is
 * already initialised, the fix is a contract amendment, and an amendment to acceptance scope needs
 * the user. The same fact is visible in the document alone: read it at `validate`, while re-cutting
 * a batch is still an edit.
 */
export function assertMigrationPlanCoverage(contract: Contract): void {
  const source = migrationInventory(contract)
  if (!source) return
  const plan = object((contract as unknown as Item).delivery_plan, 'MIGRATION_CONTRACT_INVALID')
  const batches = Array.isArray(plan.batches) ? (plan.batches as Item[]) : []
  if (!batches.length) return
  for (const [id, reader] of source.readers) {
    const requirements = ids(reader.requirement_ids, 'MIGRATION_CONTRACT_INVALID')
    const acceptance = ids(reader.acceptance_ids, 'MIGRATION_CONTRACT_INVALID', true)
    const within = batches.filter(
      (batch) =>
        covers(requirements, ids(batch.requirement_ids, 'MIGRATION_CONTRACT_INVALID', true)) &&
        covers(acceptance, ids(batch.acceptance_ids, 'MIGRATION_CONTRACT_INVALID', true))
    )
    if (within.length) continue
    const spread = batches
      .filter((batch) =>
        requirements.some((requirement) =>
          ids(batch.requirement_ids, 'MIGRATION_CONTRACT_INVALID', true).includes(requirement)
        )
      )
      .map((batch) => String(batch.id))
    throw new Error(
      `MIGRATION_READER_BATCH_COVERAGE_INVALID: reader ${id} needs ${requirements.join(
        ', '
      )} and no single batch holds them${
        spread.length > 1 ? ` — they are split across ${spread.join(', ')}` : ''
      }; admission binds a reader to one packet, so re-cut the batches or narrow the reader`
    )
  }
}
...
export function assertMigrationAdmission(contract: Contract, payload: Item): void {
  const source = migrationInventory(contract)
  /** Project the global inventory onto this admission without pretending future packets landed. */
  const migrationScope = source
    ? (() => {
        const requirements = ids(payload.requirement_ids, 'ADMISSION_REQUIREMENT_SCOPE_INVALID'),
          acceptance = ids(payload.acceptance_ids, 'ADMISSION_ACCEPTANCE_SCOPE_INVALID'),
          intersects = (entry: Item) =>
            (entry.requirement_ids as string[]).some((id) => requirements.includes(id)) ||
            (entry.acceptance_ids as string[]).some((id) => acceptance.includes(id)),
          surfaces = new Set(
            [...source.surfaces].filter(([, entry]) => intersects(entry)).map(([id]) => id)
          ),
          removed = new Set(
            [...surfaces].filter((id) => source.surfaces.get(id)!.final_disposition === 'REMOVE')
          ),
          readers = new Set(
            [...source.readers]
              .filter(
                ([, entry]) =>
                  intersects(entry) ||
                  (entry.legacy_surface_ids as string[]).some((id) => removed.has(id))
              )
              .map(([id]) => id)
          )
        return { requirements, acceptance, surfaces, removed, readers }
      })()
    : undefined
  /** A prerequisite packet can be unrelated to the contract's later migration work. */
  const touchesMigration =
    migrationScope && (migrationScope.surfaces.size > 0 || migrationScope.readers.size > 0)
  if (!source || !migrationScope || !touchesMigration) {
  ...
  const scopedReaders = new Map([...readers].filter(([id]) => migrationScope.readers.has(id)))
  ...
  const removed = [...migrationScope.removed]
  ...
  if (
    readerBindings.size !== scopedReaders.size ||
    !covers([...scopedReaders.keys()], readerBindings.keys())
  )
  ...
  for (const [id, reader] of scopedReaders) {
