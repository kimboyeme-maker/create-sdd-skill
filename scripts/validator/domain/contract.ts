...
import { resolveDerivable } from './derive/contract.ts'
...
/**
 * Parse the normative contract block, then fill in the fields the prose already determines.
 *
 * Absent derivable fields are resolved from the document before anything is checked, so a table that
 * carries a field's machine columns is allowed to be the only copy of it. A field the author wrote
 * is never overwritten — the drift check needs two independently stated values to compare.
 */
export function readContractText(
  ...
  if (parsed.protocol !== 'sdd-loop-delivery/v1') throw new Error('CONTRACT_PROTOCOL_UNSUPPORTED')
  const contract = resolveDerivable(parsed, sources.self ?? text).contract as Record<
    string,
    unknown
  >
  ...
  if (Object.hasOwn(contract, 'acceptance')) {
    ...
    for (const item of contract.acceptance) {
      ...
      // A UNIVERSAL claim asserts something about every member of a set, so the set is the claim.
      // Without it the oracle cannot be built and the reader cannot tell what was ruled out. The
      // migration path already required this for zero-reader claims; nothing required it anywhere
      // else, so a universal claim outside a migration passed with no universe at all.
      const claim = item.claim as { quantifier?: unknown; universe?: unknown } | undefined
      if (claim && typeof claim === 'object' && claim.quantifier === 'UNIVERSAL') {
        const universe = claim.universe
        if (
          !Array.isArray(universe) ||
          !universe.length ||
          universe.some((member) => typeof member !== 'string' || !member.trim())
        )
          throw new Error('CONTRACT_UNIVERSAL_CLAIM_UNIVERSE_REQUIRED')
      }
    }
  }
