 ...
 * obeyed. Mechanical defect cases are different in kind: `--mechanical` decides each one by running
 * its detector and comparing codes, which is a real verdict with no judge in it.
 */
...
if (import.meta.main) {
  ...
  for (let index = 0; index < args.length; index++) {
    ...
    else if (args[index] === '--mechanical') mechanical = true
    else {
  ...
  try {
    if (mechanical) {
      // Imported lazily so a plain suite check does not pull the detector machinery into scope.
      const { defectCases, runSuite } = (await import('./rsi.ts')) as typeof import('./rsi.ts')
